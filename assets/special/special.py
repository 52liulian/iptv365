    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if lines and lines[0].strip().startswith("#EXTM3U"): 
                ...
                lines = newlines.splitlines()  #使用转换后的内容重新赋值给 lines
            # 判断是否是m3u格式，如果是特别处理。↑↑↑↑↑↑↑

            print(f"行数: {len(lines)}")
            for line in lines:
                line = line.strip()
                if  "#genre#" not in line and line not in excudelist_lines:
                    # 查找所有 URL 模式
                    url_pattern = re.compile(r'([^,]+),(\S+://\S+)')
                    matches = list(url_pattern.finditer(line))
                    
                    if matches:
                        # 如果找到多个匹配项，每个都是一个完整的链接
                        for match in matches:
                            channel_name = match.group(1).strip()
                            url = match.group(2).strip()
                            if channel_name and url:
                                all_lines.append(f"{channel_name},{url}")
                    else:
                        # 没有找到匹配项，尝试直接添加（可能是格式不同）
